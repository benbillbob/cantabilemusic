...
SQL;
    ...
    public function getFormOptions()
    {
        ...
        $options = new FieldList(
            ...
            new TreeDropdownField("FinishPageID", "Redirect on complete", "SiteTree"),
...
, 'UserFormsConditional');
    ...
    protected function buildWatchJS($watch)
    {
        ...
        foreach ($watch as $key => $rule) {
			...
			$dataTarget = '[data-id="'.substr($target, 1).'"]';
            $result .= <<<EOS
\n
    $('.userform').on('{$events}',
    "{$selectors}",
    function (){
        if ({$operations}) {
            $('{$target}').{$rule['view']};
            $('{$dataTarget}').{$rule['view']};
        } else {
            $('{$target}').{$rule['opposite']};
            $('{$dataTarget}').{$rule['opposite']};
        }
    });
    $('{$target}').find('.hide').removeClass('hide');
    $('{$dataTarget}').find('.hide').removeClass('hide');
EOS;
        }
